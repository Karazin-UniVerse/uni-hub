...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange }) => {
  return (
    <aside className="hidden md:flex w-64 bg-[var(--kz-sidebar-bg)] flex-col shrink-0 border-r border-white/5 select-none">
      {/* Navigation items */}
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        <p className="px-3 pt-2 pb-1.5 text-[10px] font-bold text-white/40 uppercase tracking-wider">
          Розділи кабінету
        </p>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <Icon
                  size={18}
                  className={clsx(isActive ? "text-white" : "text-white/60")}
                />
      ...
      <div className="p-3 border-t border-white/10 space-y-2">
        <a
          ...
          className="flex items-center justify-between px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-white/80 hover:text-white text-xs font-medium transition-colors"
        >
